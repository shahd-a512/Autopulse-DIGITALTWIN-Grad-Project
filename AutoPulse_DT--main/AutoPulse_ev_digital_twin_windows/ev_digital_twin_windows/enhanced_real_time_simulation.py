"""
Enhanced Windows-compatible Real-time Simulation Interface for Electric Vehicle Digital Twin
------------------------------------------------------------------------------------------
...
# Call setup_paths at the beginning
setup_paths()

# Now import the EV digital twin module
try:
    from integration.ev_digital_twin import ElectricVehicleDigitalTwin
except ImportError as e:
    print(f"Error importing ElectricVehicleDigitalTwin: {e}")
    print("Trying alternative import paths...")
    
    try:
        # Try relative import
        from ev_digital_twin.integration.ev_digital_twin import ElectricVehicleDigitalTwin
        print("Successfully imported from ev_digital_twin package")
    except ImportError as e2:
        print(f"Error with alternative import: {e2}")
        print("Please check your Python path and make sure the module is available.")
        print("You may need to adjust the path in the launcher script")
        sys.exit(1)
...
class RealTimeEVSimulation:
    ...
    def __init__(self, ev_model=None):
        ...
        self.max_time = float('inf')  # No time limit for simulation
        ...
        print("Enhanced real-time EV simulation interface initialized")
    ...
    def _init_animation(self):
        ...
        self.axes[0, 0].set_xlim(0, 300)  # Start with 5 minutes view
    ...
    def export_to_csv(self, filename=None):
        ...
        if filename is None:
            # Use current directory with platform-compatible path
            filename = os.path.join(os.getcwd(), "real_time_simulation_results.csv")
...
def run_real_time_simulation():
    ...
    """
    print("Starting Enhanced EV Digital Twin Simulation...")
    ...
    try:
        ...
        output_file = os.path.join(os.getcwd(), "real_time_simulation_results.csv")
